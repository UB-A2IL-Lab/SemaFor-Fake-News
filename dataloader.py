...
class Loader(Dataset):
    def __init__(self, args, split, captioning_dataset, art2id, fake_articles):
        ...
        # # Why are fake artickles doubled for training?
        # # Not really. There is a return.
        # if split == 'train':
        #     neg_count = 0
        ...
        # There are only fake articles when testing and validating, but with real
        # ones when training. 
        if split == 'train' or args.test_with == 'fake-real':
            for i in self.realarts2id:
                name = '1_' + i
                self.arts.append(name)
            for i in self.fakearts2id:
            ...
            if args.test_with == 'fake':
                for i in self.fakearts2id:
                    name = '0_' + i
                    self.arts.append(name)
            elif args.test_with == 'real':
                for i in self.realarts2id:
                    name = '1_' + i
                    self.arts.append(name)
        # ipdb.set_trace()
    ...
    def parse(self):
        ...
        # delete items whose values are None
        # 2021-02-09
        # ipdb.set_trace()
        cap_keys = set(arts2caps.keys())
        real_keys = set(realarts2id.keys())
        fake_keys = set(fakearts2id.keys())
        common_keys = cap_keys.intersection(real_keys, fake_keys)
        c_realarts2id = {} 
        c_arts2caps = {}
        c_fakearts2id = {}
        for k in common_keys:
            c_realarts2id[k] = realarts2id[k]
            c_arts2caps[k] = arts2caps[k]
            c_fakearts2id[k] = fakearts2id[k]


        return c_realarts2id, caps2id, c_arts2caps, c_fakearts2id
